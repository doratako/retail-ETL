...
def get_unique_session_id():
    ...
    return initial_load_session_id


unique_load_session_id = get_unique_session_id()


# STAGE database
def load_stage_tables():
    ...
    # META_LOAD_HISTORY
        ...
        for file in source_files:
            meta_load_history_tbl = insert(meta_load_history).values(load_session_id=unique_load_session_id,
    ...
    # TRANSACTION
        for _, row in retail_df.iterrows():
            transaction_tbl_row = insert(transaction).values(invoice_id=row["Invoice_ID"],
                                                              ...
                                                              country=row["Country"],
                                                              load_session_id=unique_load_session_id
            )
    ...
    # TRANSACTION_LOAD_HISTORY
        for _, row in retail_df.iterrows():
            transaction_tbl_row = insert(transaction_load_history).values(invoice_id=row["Invoice_ID"],
                                                                          stock_code=row["StockCode"],
                                                                          description=row["Description"],
                                                                          quantity=row["Quantity"],
                                                                          invoice_date=row["InvoiceDate"],
    ...
    # CUSTOMER_LOAD_HISTORY
        ...
        for i in range(len(source_files)):
            meta_load_history_tbl_update = (update(meta_load_history)
                                            .where(meta_load_history.c.load_session_id == unique_load_session_id)
